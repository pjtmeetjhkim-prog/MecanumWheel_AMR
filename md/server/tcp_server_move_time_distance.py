...
class DefaultRaspbotHandler(RaspbotCommandHandler):
    ...
    def _cms_to_speed(self, velocity_cms: float, sideways: bool = False) -> int:
        ...
        실측 기반 구간별 보정:
          고속 (v >= 47 cm/s, pwm ~71+) : intercept = 15  → pwm = (v-15)/0.45
            - pwm=78 → 실측 50 cm/s ✓ (모델과 일치)
          중저속 (v < 47 cm/s)          : intercept = 17  → pwm = (v-17)/0.45
            - pwm=22 → 실측 27 cm/s  (모델 24.9, 보정 후 ≈ 정확)
            - pwm=41 → 실측 36 cm/s  (모델 33.5, 보정 후 ≈ 정확)
            - pwm=51 → 실측 40 cm/s  (보정 후 ≈ 정확)
        """
        if sideways:
            pwm = (velocity_cms - 6.0) / 0.42
        else:
            if velocity_cms >= 47.0:          # 고속: 원래 모델 그대로
                pwm = (velocity_cms - 15.0) / 0.45
            else:                             # 중저속: 실측 보정 intercept
    ...
    def _distance_compensation(self, distance_mm: float, speed: int) -> float:
        ...
        파라미터 (a, b)는 실측 오차 데이터를 최소제곱법으로 피팅:
          speed 30 : a=-0.190, b=+5.93  (측정 10개 → RSS ~17)
          speed 50 : a=-0.144, b=+3.13  (측정 10개 → RSS ~18)
          speed 80 : a=-0.099, b=+3.27  (측정 10개 → RSS ~24)

        새 speed 추가 방법:
          1. 20cm, 80cm 두 거리만 측정해 오차 e1, e2를 구한다.
          2. a = (e1 - e2) / (20 - 80) = (e2 - e1) / 60
             b = e1 - a * 20
          3. PARAMS 딕셔너리에 추가.
        """
        ...
        # ── 파라미터 테이블 {speed: (a, b)} ──────────────────────────
        # 이 값들을 바꾸면 보정 특성이 바뀜.
        # a : 거리에 비례하는 오차 보정 (기울기)
        # b : 거리와 무관한 기저 오차 보정 (절편, 가속/제동 등)
        PARAMS = {
            30: (-0.190, +5.93),
            50: (-0.144, +3.13),
            80: (-0.099, +3.27),
    ...
    def _send_motors_sync(self, motors: Dict[int, int]) -> None:
        ...
        for mid in range(4):          # 순서 보장: 0,1,2,3
            ...
            if mid > 1 and motor_spd != 0:
                motor_spd -= 1
            motor_spd = max(0, motor_spd)   # -1 → 0xFF(255) 버그 방지
    ...
    def _handle_locked(self, op: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        #
        # 필수: distance (float, mm, 양수)  ← 이동 목표 거리
        # 필수: seconds  (float, s,  양수)  ← 이 시간 안에 도달 (= 구동 시간)
        #       vx / vy / wz (float, 기본 0.0)
        # ※ speed 는 클라이언트가 보내지 않음. 서버가 distance/seconds 로 역산.
        # ──────────────────────────────────────────────────────────────────
        if op == "move_distance_time":
            ...
            # ── 필요 속도 역산: velocity = distance / time ───────────────
            sideways       = abs(vx) > abs(vy)
